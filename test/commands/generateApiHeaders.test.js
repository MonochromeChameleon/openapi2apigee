...
describe('generateApi with headers', () => {
  ...
  describe('Add header policy', () => {
    ...
    it('Headers securit headers policy should be generated', async () => {
      ...
      expect(headers.Header[0]._).toBe('max-age=31536000; includeSubDomains; preload')
    ...
    it('Target should contain header step in PreFlow', async () => {
      ...
      expect(result.TargetEndpoint.PreFlow[0].Request[0].Step[0].Name[0]).toBe('Add AWS api key header')
      expect(result.TargetEndpoint.PreFlow[0].Request[0].Step[1].Name[0]).toBe('Add token header')
    })
    ...
    it('Proxy should contain header step in PostFlow', async () => {
      ...
      expect(result.ProxyEndpoint.PostFlow[0].Response[0].Step[0].Name[0]).toBe('Add security headers')
    })
  })
})
