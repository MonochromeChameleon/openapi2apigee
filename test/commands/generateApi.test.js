...
describe('generateApi', () => {
  ...
  describe('generateSkeleton', () => {
    it('generate Skeleton should create folder structure', async () => {
      ...
      const rootFolder = await fs.lstat(path.join(options.destination, options.apiProxy))
      const proxiesFolder = await fs.lstat(path.join(options.destination, `${options.apiProxy}/apiproxy/proxies`))
      const targetsFolder = await fs.lstat(path.join(options.destination, `${options.apiProxy}/apiproxy/targets`))
      const policiesFolder = await fs.lstat(path.join(options.destination, `${options.apiProxy}/apiproxy/policies`))
    ...
    it('destination path ending with / should generate Skeleton Folder', async () => {
      ...
      const rootFolder = await fs.lstat(path.join(options.destination, options.apiProxy))
      const proxiesFolder = await fs.lstat(path.join(options.destination, `${options.apiProxy}/apiproxy/proxies`))
      const targetsFolder = await fs.lstat(path.join(options.destination, `${options.apiProxy}/apiproxy/proxies`))
      expect(rootFolder.isDirectory()).toBe(true)
      expect(proxiesFolder.isDirectory()).toBe(true)
      expect(targetsFolder.isDirectory()).toBe(true)
    })
