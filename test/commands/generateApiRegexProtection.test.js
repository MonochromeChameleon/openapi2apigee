...
describe('generateApi with regex-protection', () => {
  ...
  describe('Add regex-protection policy', () => {
    ...
    it('Proxy should contain parameter check in listPets flow', async () => {
      ...
      expect(
        result.ProxyEndpoint.Flows[0].Flow[0].Request[0].Step[0].Condition[0]
      ).toBe(
        '(request.queryparam.param1 Equals null) or (request.queryparam.param2 Equals null)'
      );
      expect(
        result.ProxyEndpoint.Flows[0].Flow[0].Request[0].Step[0].Name[0]
      ).toBe('Raise Regex Error');
    });
    ...
    it('Proxy should contain quotaAnil in listPets flow', async () => {
      ...
      expect(
        result.ProxyEndpoint.Flows[0].Flow[0].Request[0].Step[1].Name[0]
      ).toBe('quotaAnil');
    });
  });
});
