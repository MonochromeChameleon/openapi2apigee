...
const ALLOWED_VERBS = ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'HEAD', 'TRACE', 'CONNECT', 'PATCH']

function generateFlow (flows, { apiPath, method, resource, raiseFaultName }) {
  const operationId = resource.operationId || `${method.toUpperCase()} ${apiPath}`
  ...
  if (resource['x-a127-apply']) {
    Object.entries(resource['x-a127-apply']).forEach(([service, { endPoint, pipe }]) => {
...
export async function generateProxyEndPoint (apiProxy, {
  ...
  Object.entries(api.paths)
    .flatMap(([apiPath, endpoint]) => Object.entries(endpoint)
