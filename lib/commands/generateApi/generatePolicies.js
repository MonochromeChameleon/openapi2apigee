...
async function generateService(service, { rootDirectory }, api) {
  ...
  if (provider.indexOf('regex') > -1) {
    ...
    const js = join(
      __dirname,
      '../../resource_templates/jsc/JavaScriptFilter.js'
    );
    await fs.copyFile(
      js,
      join(rootDirectory, 'resources/jsc/', `${serviceName}.js`)
    );
    // regex
    const qs = join(
      __dirname,
      '../../resource_templates/jsc/querystringDecode.js'
    );
    await fs.copyFile(
      qs,
      join(rootDirectory, 'resources/jsc/', `${serviceName}-querystring.js`)
    );
    const x = regex.regularExpressions();
    await fs.writeFile(
      join(rootDirectory, 'resources/jsc/regex.js'),
      `var elements = ${JSON.stringify(x)};`,
      'utf8'
    );
  }
  ...
  if (provider.indexOf('input-validation') > -1) {
    ...
    xmlStrings.push({
      xmlString: extractVars.extractVarsGenTemplate(
        extractVarsOptions,
        extractVarsOptions.name
      ),
      serviceName: 'extractPathParameters',
    });
  }
  ...
  if (
    provider.indexOf('input-validation') > -1 ||
    provider.indexOf('output-validation') > -1
  ) {
    await fs.mkdir(`${rootDirectory}/resources/jsc`, { recursive: true });
    // output validation
    await fs.copyFile(
  ...
  if (provider.indexOf('oauth') > -1 && serviceName === 'oauth2') {
    // Add cache Policies
    xmlStrings.push({
      xmlString: oauth2.verifyAccessTokenGenTemplate(
        serviceOptions,
        'verifyAccessToken'
      ),
      serviceName,
  ...
  return Promise.all(
    xmlStrings.map(({ xmlString, serviceName: filename }) =>
      fs.writeFile(`${rootDirectory}/policies/${filename}.xml`, xmlString)
    )
  );
}
  ...
  if (api['x-a127-services']) {
    ...
    await Promise.all(
      services.map((service) =>
        generateService(service, { rootDirectory }, api)
      )
    );
  }
}
