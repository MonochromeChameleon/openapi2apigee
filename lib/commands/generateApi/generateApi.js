...
export async function generateApi (apiProxy, options) {
  ...
  const api = await new Promise((res, rej) => parser.parse(options.source, async (err, a) => {
    if (err) return rej(err, { error: 'openapi parsing failed..' })
    return res(a)
  }))
  ...
  await new Promise((res, rej) => zip5.zipFolder(srcDirectory, (err) => {
    if (err) return rej(err)
    res()
  }))

  await new Promise((res, rej) => zip5.writeToFile(destDirectory, (err) => {
    if (err) return rej(err)
    res()
  }))
}
